...
struct Coord {
  ...
  bool operator==(const Coord& other) const { return abs(x - other.x) < 1e-6f && abs(y - other.y) < 1e-6f; }
};
...
struct Simulation {
  ...
  void updateDroneXY() { CURRENT_POS = CURRENT_POS + Coord{cos(CURRENT_DIR), sin(CURRENT_DIR)} * CURRENT_SPEED * simulationTimeStep; }
};
